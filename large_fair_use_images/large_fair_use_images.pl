...
WHERE EXISTS (
  SELECT 1 FROM image WHERE pt.page_title=img_name AND IMG_MAJOR_MIME='image' AND ((IMG_WIDTH*IMG_HEIGHT)>100000 OR
( IMG_WIDTH > IMG_HEIGHT AND IMG_WIDTH>1000 ) OR 
( IMG_WIDTH < IMG_HEIGHT AND IMG_HEIGHT>1000 ) OR
( IMG_WIDTH = IMG_HEIGHT AND IMG_HEIGHT>1000 )
)
);
));
...
== 비자유 로고 ==
* 참조: [[:분류:비자유 로고]]
* 기준: 가로, 세로 중 긴 것을 기준으로 1,000 픽셀을 초과 또는 가로 세로의 곱이 100,000 픽셀을 초과 (저장 기준임. 표시 기준의 2배)
);
...
== 비자유 건축물 ==
* 참조: [[:분류:비자유 건축물]]
* 기준: 가로가 400 픽셀, 세로가 1,200 픽셀을 초과 또는 가로 세로의 곱이 200,000 픽셀을 초과 (저장 기준임. 표시 기준의 2배)
);
$final_result .= sprintf($report_template1, join("\n", @output2));
...
* 기준: 가로 세로의 곱이 80,000 픽셀을 초과 (저장 기준임. 표시 기준의 2배)
);
$final_result .= sprintf($report_template1, join("\n", @output4));
