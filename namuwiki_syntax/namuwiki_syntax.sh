...
URL3='https://ko.wikipedia.org/w/index.php?title=%ED%8A%B9%EC%88%98:%EA%B2%80%EC%83%89&limit=5000&offset=0&ns0=1&search=insource%3A%2F%5C%5B%5C%5Bhttps%2A%3A%5C%2F%5C%2F%2F'
CNT=1
...
== 외부 링크가 더 시드 문법으로 걸린 문서 ==
{| class="wikitable" style="text-align:center; width: 750px;"
! width="300px" | 더 시드 문법
! width="450px" |[[미디어위키]] 문법
|-
|<code><nowiki>[[http://example.org|예제]]</nowiki></code>||<code><nowiki>[http://example.org 예제]</nowiki></code>
|-
...
IFS='
'
for sth in `wget -qO- $URL3 | perl -pe 's/title="/\ntitle="/mg' | grep -P '^title=' | perl -pe 's/(^title="|" +data-serp-pos=.*)//g' | grep -vP '>|\[' | perl -pe 's/^\*+$|(More options|더 많은 옵션)"//'`
do
	echo "| $CNT || [[${sth}]]"
	echo "|-"
	CNT=`expr $CNT + 1`
